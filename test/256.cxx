...
inline std::ostream &operator<<(::std::ostream &out, u512_t v) {
  const ::std::ostream::sentry sentry(out);
  const auto flags = out.setf(::std::ios_base::hex, ::std::ios_base::basefield);
  out << v.h << "." << v.g << "." << v.f << "." << v.e << "." << v.d << "." << v.c << "." << v.b << "." << v.a;
  out.setf(flags);
  return out;
}
...
extern "C" __dll_export __hot void mulset_256x256_256(u256_t *__restrict r, const u256_t *x, const u256_t *y) {
  using namespace erthink;
#define MUL(X, Y) uint64_t H##X##Y, L##X##Y = mul_64x64_128(x->X, y->Y, &H##X##Y)
  MUL(a, a);
  MUL(a, b);
  MUL(a, c);
  MUL(a, d);
...
extern "C" __dll_export __hot void mulset_256x256_512(u512_t *__restrict r, const u256_t *x, const u256_t *y) {
  using namespace erthink;
#define MUL(X, Y) uint64_t H##X##Y, L##X##Y = mul_64x64_128(x->X, y->Y, &H##X##Y)
  MUL(a, a);
  MUL(a, b);
...
TEST(mul_demo, u256x256_512) {
  ...
  const u256 x = {{0x0029D5041B07BBC5, 0x0013E3DD1DE9F1B1, 0x00655CC73C703B63, 0x00BE768C6FD17F49}};

  // fedcba9876543210fedcba9876543210fedcba9876543210fedcba9876543210
  // 0086A17845E30DFF008E2A7B0B2274FD008C9C281884AD5700A7B54C6A22E2B5
  //                 fedcba9876543210    fedcba9876543210    fedcba9876543210
  const u256 y = {{0x00A7B54C6A22E2B5, 0x008C9C281884AD57, 0x008E2A7B0B2274FD, 0x0086A17845E30DFF}};
  ...
  //                  fedcba9876543210    fedcba9876543210    fedcba9876543210
  const u512 xy = {{0x04EF0E067265AC49, 0x8DF65EE05A27335F, 0xB5826F7887CF4A4C, 0x25879513C3B3EE9A, 0xFEA3AEC8D71927DA,
                    0x06D9ECA5D5B9B4BB, 0x2DDF07EF6C05E687, 0x0000642A2F8C6AB1}};
